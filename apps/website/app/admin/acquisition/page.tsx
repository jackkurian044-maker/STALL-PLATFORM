...
export default function AcquisitionPage() {
  ...
  <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">{[["DISCOVERED","Discovered"],["SHORTLISTED","Shortlisted"],["CONTACTED","Contacted"],["INTERESTED","Interested"]].map(([k,l])=><div key={k} className="rounded-2xl border border-black/10 bg-white p-4"><div className="text-2xl font-semibold">{counts[k] ?? 0}</div><div className="text-xs text-black/50">{l}{c.status==="FOLLOW_UP"&&<button onClick={()=>action(c.id,"outreach")} className="rounded-lg border border-green-300 bg-green-50 px-3 py-2 text-xs font-semibold text-green-800">WhatsApp</button>}{c.status==="INTERESTED"&&<button onClick={()=>setStatus(c.id,"READY_FOR_STALL")} className="rounded-lg bg-[#0f1a24] px-3 py-2 text-xs font-semibold text-white">Ready for STall</button>}{c.status==="READY_FOR_STALL"&&<button onClick={()=>action(c.id,"handoff")} className="rounded-lg bg-emerald-700 px-3 py-2 text-xs font-semibold text-white">Hand off to STall-App</button>}</div></div>)}</div>
